...
class ImageSpec(BaseSpec):
    def get_repos(self):
        ...
        return {
            ...
            'urls': r['urls'] + self.get_cf_list('repos::urls'),
        }

    def __mergelist(self, name):
        return self.get_cf_list('OSBASE::'+name)+self.get_cf_list('rootfs::'+name)
    ...
    def get_purged_files(self):
        return self.get_purged_locales()+self.__mergelist('post-clean::remove')

    def get_empty_dirs(self):
        return self.__mergelist('post-clean::empty-dirs')

    def load_spec(self, spec):
        ret = BaseSpec.load_spec(self, spec)
        self['VERSION'] = lambda x: basename(x)
        return ret
